...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN: AHMAD GUSTIAN ADI N\n' 
            + 'ORG: OWNER GUSTIAN BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6282137409151:+62 821-3740-9151\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal =10
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6282137409151@s.whatsapp.net"] 
/************************************/
          	...
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return client.sendMessage(from,`Limit request anda sudah habis, limit di reset setiap jam 21.00!\n\n_Note : limit bisa di dapatkan dengan cara ${prefix}buylimit dan dengan naik level_`, text,{ quoted: mek})
   ...
   if (isLimit(sender)) return reply(ind.limitend(pusname)) 
     ...
     reply(mess.wait)
           ...
           if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
                tahta = `${body.slice(7)}`
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 7) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.zeks.xyz/api/hartatahta?text=${tahta}&apikey=apivinz`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek, caption: `Harta Tahta ${tahta}`})
                  await limitAdd(sender) 
                  break  
                  case 'steel3d':
                if (!isRegistered) return reply(ind.noregis())
    ...
    break
			...
			case 'admin':
                 ...
                 tod = await getBuffer(`https://i.ibb.co/9thhyGd/Ahmad-Gustian-Adi-Nugraha.jpg`)
                ...
                await reply('Mencari pasangan.....')
                await reply(`wa.me/${anug}`)
                await reply( `Mitra di temukan: 🙉\n*${prefix}next* — Temukan pasangan baru!!`)
            break
            case 'next':
                if (!isRegistered) return reply( ind.noregis())
                if (isGroup) return  reply( 'Command ini tidak bisa digunakan di dalam grup!')
            ...
            break
                ...
                addKoinUser('6282137409151@s.whatsapp.net', fee)
           ...
           case 'demote':
              ...
              case 'kicktime':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					client.sendMessage(from, 'Udah gue keluarin hahahahahha', text)
					}, 31000)
					setTimeout( () => {
					reply('Done Min:v')
					}, 30000)
					...
					setTimeout( () => {
					client.sendMessage(from, `Bismilah Kick @${mentioned[0].split('@')[0]} aowkwowk`, text) // ur cods
					}, 24000)
					setTimeout( () => {
					client.sendMessage(from, 'Gw Kick Sekerang Ya Min', text)
					}, 18000)
					setTimeout( () => {
					client.sendMessage(from, 'Kick Skrg Min? Apa kasih Waktu Lagi?', text)
                    }, 14000)
					setTimeout( () => {
					client.sendMessage(from, `Dah Siap Blm Ngab @${mentioned[0].split('@')[0]}`, text)
					}, 8000)
					setTimeout( () => {
					client.sendMessage(from, 'Otw Kick Min Tapi Gw Kasih Waktu Dlu Ya Min', text)
					}, 2500)
					setTimeout( () => {
				...
				case 'clone':
			  	...
			  	case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸a𝗻 EVENT 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️*')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'setppbot':
					...
					reply('Makasih Dah Mengganti foto profil ku min')
					break 
				case 'setpp': 
                        if (!isGroup) return reply(ind.groupo())
                       if (!isGroupAdmins) return reply(ind.admin())
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('Ah','black'), 'Gustian bot', color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
